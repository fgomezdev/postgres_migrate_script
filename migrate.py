...
def migrate_table_structure(source_cursor, source_table_name, destino_cursor, target_schema, target_table_name):
    ...
    destino_cursor.execute(sql_create_table)
...
try:
    for i, _source_table_name in enumerate(tables_to_migrate):
        ...
        migrate_constraints(_source_cursor, _source_table_name, _target_cursor, TARGET_SCHEMA, _target_table_name, PREFIX)
        migrates_sequences(
            _source_cursor, SOURCE_SCHEMA, _source_table_name, _target_cursor, TARGET_SCHEMA, _target_table_name, prefix=""
        )

        # Comentado por el momento la importación de datos hasta tanto se resuelva el error con los distintos tipos recibidos
        migrate_data(_source_cursor, SOURCE_SCHEMA, _source_table_name, _target_cursor, TARGET_SCHEMA, _target_table_name)
